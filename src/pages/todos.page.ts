...
export class TodosPage extends LoginPage {
  ...
  private deleteTaskButton =
    'li.list-group-item button.btn.btn-danger:has-text("X")';
  ...
  private getTodoItemLocator(todoText: string): Locator {
    return this.page
      .locator("li.list-group-item:not(.fade-leave-active):not(.fade-leave)")
      .filter({
        has: this.page.locator("div.col-sm-11.text-left", {
          hasText: todoText,
        }),
      });
  }
